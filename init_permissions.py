...
def init_permissions_system():
    """Initialize permissions system with tables and default data"""
    ...
    try:
        ...
        for role_key, role_data in DEFAULT_ROLES.items():
            try:
                ...
                if role_data['permissions'] == 'all':
                    ...
                    for perm in all_permissions:
                        cursor.execute("""
                            ...
                            VALUES (%s, %s)
                        """, (role_id, perm['id']))
                    print(f"  ✓ {role_data['name']} (all permissions)")
                else:
                ...
                print(f"  - {role_data['name']}: {e}")
        
        conn.commit()
        
        # Update existing employees with roles
        print("\nUpdating existing employee roles...")
        cursor.execute("SELECT id, role FROM employees")
        employees = cursor.fetchall()
        for emp in employees:
            if not emp['role'] or emp['role'] == 'NULL':
                cursor.execute("UPDATE employees SET role = 'employee' WHERE id = %s", (emp['id'],))
                print(f"  ✓ Set default role for employee ID {emp['id']}")
        
        conn.commit()
        
        print("\n" + "="*50)
        print("✓ Permissions system initialized successfully!")
        print("="*50)
        ...
        print(f"  - Role-Permission mappings created")
        
    except Exception as e:
        conn.rollback()
        print(f"\n❌ Error initializing permissions system: {e}")
    finally:
        cursor.close()
        conn.close()
